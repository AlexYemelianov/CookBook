...
class App extends Component {
  ...
  remove = (index) => {
    axios({
      ...
      } else throw new Error("Рецепту немає!");
    }).catch(error => console.error('Error', error));
  }
    ...
    return (
      <Router>
        <div className="App">
          <Header />
          <hr />
          <Link to="/add"><button className="btn btn-primary btn-lg currBtnApp">Додати рецепт</button></Link>
          <Link to="/search"><button className="btn btn-primary btn-lg currBtnApp">Знайти рецепт</button></Link>
          <hr />
          <div className="row justify-content-around">
            ...
            <Route path="/add" render={() => <RecipeForm type="add" fields={{ title: ["text", "Назва рецепту"], url: ["text", "URL картинки"], description: ["text", "Як готувати"] }} addRecipe={this.add.bind(this)} />} />
